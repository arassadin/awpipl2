...
AWPRESULT _awpLoadAWPImageV1(const char* lpFileName, awpImage** ppImage)
{
   ...
   DWORD dwPixelSize = 0;
   /*header variables*/
   DWORD dwMagic; /*magic number. must be 0x0A0D5C */
   AWPBYTE  bVersion; /*version number. must be 1*/
   WORD  wSizeX;   /*width of the image. Can be 1-65535*/
   WORD  wSizeY;   /*height of the image. Can be 1-65535*/
   DWORD dwType;   /*Type of the image. Can be
                        0 - AWP_COLOR
                        1 - AWP_GRAYSCALE
                        2 - AWP_SHORT
                        3 - AWP_PALETTE
                        4 - AWP_FLOAT*/
   DWORD dwSize;     /*size of pixels*/
   AWPBYTE* pbBuffer;/*buffer for pixels*/
   /*awp image creation variables*/
   AWPBYTE  bNumChannels; /*number of channels*/
   DWORD dwTypeImage;  /*type of image*/
...
AWPRESULT _awpLoadJPEGImage(const char* lpFileName, awpImage** ppImage)
{
 ...
 //   res = AWP_OK;/*initialize result */
    ...
    _CHECK_RESULT_(( res=awpCreateImage(&pImage,(WORD)cinfo.output_width,(WORD)cinfo.output_height,iChannels,type_image)))
