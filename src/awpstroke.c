...
AWPRESULT awpStrObjSquare(const awpStrokeObj* s, AWPINT* Sq)
{
    ...
    *Sq = 0;
    ...
    for (i =0; i < (AWPINT)s->Num; i++){
        *Sq +=1+s->strokes[i].xr - s->strokes[i].xl;
    }
...
static void ownFindNearXlY1(awpStrokeObj* s0, AWPINT index, ownNearSt*  s_near)
{
    ...
    s_near->x = -1;		// левая координата | для варианта, когда     +----+
    s_near->index = -1;	// индекс			|							+----------+
    // т.е. штрих, наиболее близкий по левой координате
    s_near->x1=-1;		// правая координата	| для варианта, когда	         +---+
    s_near->index1=-1;	// индекс				|						+-----------+
    // т.е. штрих, наиболее близкий по правой координате
    Min = 100000; 		// начальный порог расстояния поиска по левой координате
    Min1 = 100000; 		// ... по правой координате
...
AWPRESULT awpGetObjCountour(const awpStrokeObj* s, awpContour* c)
{
    ...
    /*Преобразуем массив штрихов в массив штрихов с атрибутами*/
    s_attr = (ownStrokeAttr*)malloc(s->Num*sizeof(ownStrokeAttr));
    for (i = 0; i < (AWPINT)s->Num; i++)
    {
        s_attr[i].y = s->strokes[i].y;
        /*рассматриваем правый конец штриха*/
        ...
        /*рассматриваем левый конец штриха*/
        ...
        {	// найден единственный перекрывающий штрих
    ...
    /*преобразуем массив аттрибутов в контур*/
...
AWPRESULT awpGetObjCentroid(const awpImage* pImg, const awpStrokeObj* pObj, awpPoint* p)
{
    ...
    p->X = floor(x0 / (double)sum + 0.5);
    p->Y = floor(y0 / (double)sum + 0.5);
...
/*
    AWPRESULT res;
    AWPDOUBLE    Intensity;
    AWPINT i,j,w,x,y,point_counter;
    AWPBYTE* p0;
    awpImage* pcopy;
    AWPDOUBLE x0,y0,p0_copy;
    ...
    _CHECK_RESULT_((res = awpCopyImage(pImg,&pcopy)));

    if (pObj == NULL || pObj->Num == 0){
        res = AWP_BADARG;
        _ERR_EXIT_
    }
    _CHECK_RESULT_((res = awpGetObjIntensity(pcopy,pObj,&Intensity)));

    if (Intensity > AWP_EPS)
    {
        p0 = (AWPBYTE*)pcopy->pPixels;
        w = pcopy->sSizeX;
        x0 = 0.0;
        y0 = 0.0;
        point_counter=0;
    ...
    if(point_counter!=0){
        p->X = (AWPINT)(x0/point_counter);
        p->Y = (AWPINT)(y0/point_counter);
    }else {
        res = AWP_BADARG;
        _ERR_EXIT_
    }

    _CHECK_RESULT_((res=awpReleaseImage(&pcopy)));
...
AWPRESULT awpGetObjOrientation(const awpImage* pImg, const awpStrokeObj* pObj, AWPDOUBLE* teta, AWPDOUBLE* mi, AWPDOUBLE* ma)
{
    ...
    *ma   = 0;
    ...
    m11 = 0; m20 = 0; m02 = 0;
    /*find the moments*/
    _CHECK_RESULT_((res = awpGetObjMoment(pImg, pObj, 1,1, &m11)))
    _CHECK_RESULT_((res = awpGetObjMoment(pImg, pObj, 2,0, &m20)))
    _CHECK_RESULT_((res = awpGetObjMoment(pImg, pObj, 0,2, &m02)))

    /*find teta*/
    if (m02 == m20)
        *teta = 3.14159 / 2.0;
    else
        *teta = 0.5*atan(2.0*m11 / (m20 - m02));
    /*major axis lenght*/
    *ma = m20*cos(*teta)* cos(*teta) + 2*m11*cos(*teta)*sin(*teta) + m02*sin(*teta)*sin(*teta);
    *ma = 12*(*ma) + 1;
    if (*ma < 0)
        *ma = 0;
    else
        *ma = sqrt(*ma);
    /*minor axis length*/
    *mi = m20*sin(*teta)*sin(*teta) - 2*m11*cos(*teta)*sin(*teta) + m02*cos(*teta)* cos(*teta);
    *mi = 12*(*mi) + 1;
    if (*mi < 0)
        *mi = 0;
    else
        *mi = sqrt(*mi);
    /*since atan returns value in the range -pi/2 to pi/2
     convert it to degrees*/
    *teta = 180.0*(*teta) / 3.14159;

CLEANUP:
    return res;
}

AWPRESULT awpDrawStrokes(awpImage** ppimg, WORD w, WORD h, AWPINT num, awpStrokeObj* pobjs)
{
    AWPRESULT res;
    AWPINT i,k;
    unsigned long j;
    ...
    for ( i=0; i<num; i+=1 ){
        ...
        for( j=0; j<cur_obj->Num; j++){
            ...
            for(k=cur_str->xl;k<=cur_str->xr;k++){
                if( (WORD)k< w && k>=0 && cur_str->y>=0 && cur_str->y<h){
