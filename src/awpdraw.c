...
AWPRESULT _awpDrawEllipse(awpImage* pImage, awpPoint center, WORD width, WORD height, AWPDOUBLE angle, AWPBYTE bChan, AWPDOUBLE dValue)
{
	AWPRESULT res = AWP_OK;
	AWPINT y;
	AWPDOUBLE diskr;
	...
	for(y = -l; y <= l; y++)
	{
		...
		if (!draw_start)
		{
			p_r.X = center.X + (WORD) ((y * e + sqrt(diskr)) / c);
			if (p_r.X > pImage->sSizeX) p_r.X = pImage->sSizeX;
			if (p_r.X < 0) p_r.X = 0;
			p_l.X = center.X + (WORD) ((y * e - sqrt(diskr)) / c);
		...
		p.X = center.X + (WORD) ((y * e + sqrt(diskr)) / c);
...
AWPRESULT _awpDrawEllipseV2(awpImage* pImage, awpPoint center, WORD width, WORD height, AWPDOUBLE angle, AWPBYTE bChan, AWPDOUBLE dValue)
{
	AWPRESULT res = AWP_OK;
	...
	while(phase <= 2 * AWP_PI)
	{
		...
		p.Y = center.Y - (WORD) (W_S * C_T - H_C * S_T);
		if (p.Y > pImage->sSizeY) p.Y = pImage->sSizeY;
		if (p.Y < 0) p.Y = 0;

		p.X = center.X + (WORD) (W_C * C_T + H_S * S_T);
		if (p.X > pImage->sSizeX) p.X = pImage->sSizeX;
		if (p.X < 0) p.X = 0;
	...
	else
	{
			...
			(pContour->Points[0].Y == pContour->Points[pContour->NumPoints - 1].Y))
		{
			...
			if ((rect.left != rect.right) && (rect.top != rect.bottom))
			{
				...
				cross_xs = (SHORT *) malloc(pContour->NumPoints * sizeof(SHORT));
				if (cross_xs == NULL)
					_ERROR_EXIT_RES_(AWP_BADMEMORY)
...
*/
					...
					while (cur_y <= max_y)
					{
						...
						{
							if (verges[i].k != 0)
								cross_xs[crs_x_count] = (SHORT) ((AWPDOUBLE) (cur_y - verges[i].bottom.Y) / verges[i].k) + verges[i].bottom.X;
