...
static AWPRESULT _awpFlipVert(awpImage** ppImageSrc)
{
    ...
    case AWP_SHORT:  iBytesCount = sizeof(SHORT)/sizeof(AWPBYTE);  break;
...
AWPRESULT awpResize(awpImage* pImageSrc, const WORD wNewWidth, const WORD wNewHeight)
{
...
AWPRESULT awpRescale(awpImage *pImageSrc, const AWPFLOAT fWidthScaleFactor, const AWPFLOAT fHeightScaleFactor)
{
    ...
    wNewWidth  = (WORD)(pImageSrc->sSizeX * fWidthScaleFactor  + 0.5);
    wNewHeight = (WORD)(pImageSrc->sSizeY * fHeightScaleFactor + 0.5);
...
AWPRESULT awpRotate( awpImage *pImageSrc, const SHORT sAngle )
{
    WORD wNewRowsCount;
    WORD wNewColsCount;
    WORD wOldRow;
    ...
    wNewHeight = (WORD)(fabs(pImageSrc->sSizeX*dSinAngle) + fabs(pImageSrc->sSizeY*dCosAngle));
    wNewWidth  = (WORD)(fabs(pImageSrc->sSizeX*dCosAngle) + fabs(pImageSrc->sSizeY*dSinAngle));
...
AWPRESULT awpRotateCenter2( awpImage *pImageSrc, const AWPDOUBLE dCosAngle, const AWPDOUBLE dSinAngle, SHORT xCenter, SHORT yCenter )
{
    WORD wNewRowsCount;
    WORD wNewColsCount;
    AWPINT wOldRow;
...
AWPRESULT awpResizeBilinear(awpImage* pImageSrc, const WORD wNewWidth, const WORD wNewHeight)
{
    ...
    AWPINT iBytesCount;
    AWPINT iImageType;
	WORD cols, rows; //for increase speed
	AWPBYTE chan_count; //for increase speed
	DWORD chan_bytes; //for increase speed
	WORD x, y;
	...
	AWPDOUBLE kx, ky;
	AWPDOUBLE sx, sy;
	DWORD addr_1, addr_2, addr_3, addr_4;
...
AWPRESULT awpResizePiecewiseCubic(awpImage* pImageSrc, const WORD wNewWidth, const WORD wNewHeight)
{
	...
	DWORD base_addr, addr_1, addr_2, addr_3, addr_1_d, addr_2_d, addr_3_d;
...
AWPRESULT awpResizeBicubic4x4(awpImage* pImageSrc, const WORD wNewWidth, const WORD wNewHeight)
{
    ...
    AWPINT iImageType;
	WORD cols, rows; //for increase speed
	WORD max_x, max_y; //for increase speed
	AWPBYTE chan_count; //for increase speed
	WORD x, y;
	WORD x_min, y_min;
	AWPBYTE c;
	DWORD base_addr, c_base_addr, addr_11, addr_12, addr_13, addr_14,
...
AWPRESULT awpResizeBicubic2x2(awpImage* pImageSrc, const WORD wNewWidth, const WORD wNewHeight)
{
	...
	AWPBYTE *pSrcMap = NULL;
	WORD cols, rows; //for increase speed
	WORD dec_cols, dec_rows; //for increase speed
	AWPBYTE chan_count; //for increase speed
	...
	WORD x_min, y_min;
	AWPBYTE c;
	DWORD base_addr, addr_11, addr_12, addr_21, addr_22,
	...
	case AWP_BYTE:
		...
		for(x = 0; x < cols; x++)
			for(y = 0; y < rows; y++)
			{
				...
				daddr_11 = (DWORD) pSrcMap + addr_11 * sizeof(PtDerivs);
				addr_11 *= iBytesCount;
				for(c = 0; c < chan_count; c++)
				{
					if (x == 0)
					...
					else
					{
						...
						{
							((PtDerivs *) daddr_11)->dy = ((SHORT)pDataSrcBYTE[addr_11 + cols * chan_bytes] - (SHORT)pDataSrcBYTE[addr_11 - cols * chan_bytes]) / 2.;
	...
	for(y = 0; y < wNewHeight; y++)
	{
		...
		for(x = 0; x < wNewWidth; x++)
		{
			...
			if (recalc)
			{
				...
				daddr_11 = (DWORD) pSrcMap + addr_11 * sizeof(PtDerivs);
				daddr_12 = (DWORD) pSrcMap + addr_12 * sizeof(PtDerivs);
				daddr_21 = (DWORD) pSrcMap + addr_21 * sizeof(PtDerivs);
				daddr_22 = (DWORD) pSrcMap + addr_22 * sizeof(PtDerivs);
...
AWPRESULT awpRotateBilinear(awpImage *pImageSrc, const short sAngle)
{
    ...
    int iImageType;
	WORD cols, rows; //for increase speed
	BYTE chan_count; //for increase speed
	WORD x_min, x_max, y_min, y_max;
	BYTE c;
	double coeffs[4];
	DOUBLE val;
    ...
    BYTE *pDataSrcBYTE = NULL;
    BYTE *pDataDstBYTE = NULL;
    ...
    memset((void*)dDir, 0, 7*sizeof(DOUBLE));
    dDir[0] = 0.0;
    dDir[1] = dCosAngle;
    dDir[2] = dSinAngle;
    dDir[3] = (sAngle < 0)?-pImageSrc->sSizeX*dSinAngle:0;
    ...
    switch(iImageType) {
    case AWP_BYTE :  iBytesCount = 1; break;
    case AWP_SHORT:  iBytesCount = sizeof(SHORT)/sizeof(BYTE);  break;
    case AWP_FLOAT:  iBytesCount = sizeof(FLOAT)/sizeof(BYTE);  break;
    case AWP_DOUBLE: iBytesCount = sizeof(DOUBLE)/sizeof(BYTE); break;
	...
	pDataSrcBYTE = (BYTE *) pImageSrc->pPixels;

	if((pDataDstBYTE = (BYTE*)malloc(wNewWidth*wNewHeight*pImageSrc->bChannels*iBytesCount*sizeof(BYTE))) == NULL)
		_ERROR_EXIT_RES_(AWP_NOTENOUGH_MEM)
    ...
    for(wNewRowsCount = 0; wNewRowsCount<wNewHeight; ++wNewRowsCount)
    {
        ...
        for( wNewColsCount = 0; wNewColsCount < wNewWidth; ++wNewColsCount)
        {
			...
			{
                memset(&pDataDstBYTE[iIndex], 0, pImageSrc->bChannels*iBytesCount*sizeof(BYTE));
				iIndex += chan_count*iBytesCount*sizeof(BYTE);
			}
			else
			{
