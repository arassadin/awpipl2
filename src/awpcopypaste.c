...
AWPRESULT awpCopyRect (const awpImage* pSrcImage, awpImage** ppDstImage, const awpRect* pRect)
{
    ...
    case AWP_BYTE:
        ...
        break;
    case AWP_SHORT:
        PixelSize = sizeof(SHORT)*pSrcImage->bChannels;
        break;
    case AWP_FLOAT:
...
AWPRESULT awpShlIntegral( awpImage* pSrc, awpImage** ppDst, AWPINT type )
{
    ...
    case AWP_LINEAR:
    case SQUARE:
        _CHECK_RESULT_((res = awpCreateImage(ppDst, (WORD)(pSrc->sSizeX + 1), (WORD)(pSrc->sSizeY + 1),
            pSrc->bChannels, AWP_DOUBLE)));
    
        break;
    case RLINEAR:
    case RSQUARE:
        _CHECK_RESULT_((res = awpCreateImage(ppDst, pSrc->sSizeX, (WORD)(pSrc->sSizeY + 1),
            pSrc->bChannels, AWP_DOUBLE)));
