...
//    else if (strcmp(lpExt, "bmp") == 0 || strcmp(lpExt,"BMP")  == 0)
//        res = _awpSaveBMPImage(lpFileName, pImage);
#ifdef HAVE_JPEG
...
AWPRESULT awpJpegToImage( const AWPBYTE* pJpegData, DWORD length, awpImage** ppImage )
{
    AWPRESULT res;
    ...
    /* create awp image */
    _CHECK_RESULT_((res = awpCreateImage(ppImage,(WORD)in_jpeg.output_width,(WORD)in_jpeg.output_height,(AWPBYTE)(in_jpeg.out_color_space==JCS_RGB?3:1),AWP_BYTE)))
...
AWPRESULT awpImageToJpeg( const awpImage* pImage, AWPBYTE** ppJpegData, DWORD* length, AWPBYTE quality )
{
    AWPRESULT res;
